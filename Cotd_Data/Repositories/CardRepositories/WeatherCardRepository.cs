...
public class WeatherCardRepository : CardRepository<WeatherCardData>, IWeatherCardRepository
{
	public WeatherCardRepository(string dataPath) : base(dataPath)
	{
	}

	public override WeatherCardData GetOne(string id)
	{
	...
	public void UpdateCard(string id, string name, string description, string image, int envoyCost)
	{
		...
		WeatherCardData? card = datas.FirstOrDefault(c => c.Id == id);
		...
		DataSaver<WeatherCardData>.Save(datas, dataPath);
	}
}
